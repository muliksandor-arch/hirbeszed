...
- téma kiválasztása;
- közös hírfolyamnézetek: Személyes, Időrend, Kedvelt;
- kedvelt hírek és előzmények.

A Hírfolyam és a Felolvasó közös hírfolyamnézet-váltója `Személyes / Időrend / Kedvelt`. Az alapértelmezett nézet a `Személyes`. A `Személyes` preferencia szerinti, időrendi hírfolyam; az `Időrend` nem személyre szabott, időrendi hírfolyam; a `Kedvelt` csak a szívecskével jelölt híreket mutatja időrendben. Az olvasott hírek a Személyes és Időrend nézetben is a feedben maradnak, csak olvasott vizuális állapotot kapnak.

Új hír érkezésekor a Hírfolyam nem ugrik automatikusan. Felülről beúszó, lebegő értesítés jelenik meg `N új hír érkezett · Megtekintem` szöveggel és frissítés ikonnal. Megtekintés után az új hírek a lista tetején jelennek meg, a hozzájuk tartozó kártyák finom `Új` jelölést kapnak. A jelölés batch-szinten tűnik el, amikor a felhasználó végiggörgette az összes újnak jelölt hírt.

Az Értesítések menüben a `Napi összefoglaló` és a `Csendes időszak` nem része az aktuális prototípus-iránynak. A megmaradó két felhasználói kapcsoló: `Rendkívüli hírek` és `Személyes hírösszefoglaló`. A rendkívüli hírek kapcsolója a valódi rendkívüli híreket és az időjárási riasztásokat fogja össze; ha be van kapcsolva, ez külön, azonnali értesítési ág, függetlenül a személyes hírösszefoglaló 2 órás szabályától. A személyes hírösszefoglaló küszöb-alapú: elsőként 10 új személyes hírnél jelenhet meg, aktív értesítés esetén ugyanaz az értesítés frissül. Új hangos értesítés csak akkor lehet, ha az utolsó hangos értesítés óta eltelt legalább 2 óra, és azóta legalább 10 újabb személyes hír gyűlt össze. Ha a 2 óra már eltelt, de még nincs meg a +10 hír, az app tovább vár, és a 10. új hír érkezésekor küldi a hangos értesítést; innen indul újra a következő 2 órás ablak. A panelben a két kapcsoló SVG ikont kap, alattuk pedig egy `Működés` kártya írja le külön a rendkívüli és a személyes értesítési szabályt.

Helyi prototípus-teszteléshez a `simulateFreshNews=3` URL-paraméter mesterséges friss hír batch-et hoz létre. Ez kizárólag ellenőrzési segéd, a végleges appban az RSS/backend frissítés adja majd ugyanezt az eseményt.

A témachip-sor első, mindig elérhető összesítő eleme `Minden téma`. Ez nem külön téma és nem frissességi nézet, hanem azt jelenti, hogy az aktuális hírfolyamnézeten belül nincs témaszűrés. A konkrét témachipek ugyanazon nézet listáját szűrik tovább.
...
A Hírfolyam jobb felső fejlécében csak a `Keresés` ikon jelenik meg. A keresés ikon az alkalmazás accent színét használja, hogy világos témában is jól látható legyen. A kedvelt hírek és az olvasott/olvasatlan állapot kezelése nem külön fejlécgomb: ezek a hírkártyákon, illetve a kapcsolódó listanézetekben maradnak.
...
Az akcióikonok inline SVG-k. A Kedvelés szívkontúrt használ, aktív állapotban tömör coral kitöltéssel. A Megosztás ikon lekerekített papírrepülő/küldés jel, külön coral mozgáscsík nélkül. Az Olvasatlan/Olvasott állapot nyitott szemmotívum: olvasatlanul semleges kontúr és üres pupilla, olvasottként a teljes szemikon coral színű pipa nélkül. A szemrajz optikailag a többi ikonnal egymagasságúra van méretezve, miközben a vizuális ikonkeret a teljes gombsorban 16 px, meta sorban 12 px, kiskártyás meta sorban 11 px marad.
...
A Felolvasó és Asszisztens oldalak jobb felső marketing CTA gombjai előfizetési állapot szerint változnak. A döntési tábla külön fájlban van rögzítve: `ELOFIZETESI_GOMB_MATRIX.md`. A gomb nem technikai ikon, hanem előfizetésre vezető szöveges ajánlat. Ingyenes használat állapotban a Felolvasó oldalon a Felolvasó funkció, az Asszisztens oldalon az Asszisztens funkció aktiválását ajánlja. AI Felolvasó előfizetés vagy próba alatt a következő ajánlat az Asszisztens funkció aktiválása. AI Asszisztens aktív előfizetésnél nincs további marketing CTA gomb, ezért az Asszisztens jobb felső fejlécének akcióterülete üres.

Az Asszisztens módváltása nem jobb felső fejlécikonon keresztül történik. A képernyő tetején belül marad a `Beszéd / Cset` módválasztó; ez az egyetlen módváltó vezérlő. A Cset mód a korábbi Néma mód működését viszi tovább: gépelt kérdés, írott válasz, hangos felolvasás nélkül. A korábbi Gépelés mód megszűnt. A Cset ikonja SVG alapú üzenetbuborék, finom coral belső vonalakkal. Az AI Asszisztens heti 5 kérdéses próba CTA-ként vagy promóként megjelenhet, de a CTA-felületen nem kell maradék kérdésszámot mutatni.

A `Beszéd / Cset` módváltó a hírfolyamnézet-váltó szegmentált dizájnnyelvét és teljes magasságát használja: teljes szélességű, kétoszlopos, surface-2 hátterű vezérlő, aktív surface állapottal. Az Asszisztens módgombokon ettől eltérően megmaradnak a Beszéd és Cset ikonok.

Cset módban nincs külön coral színű `Cset` címsor a módválasztó alatt. A csetpanel közvetlenül a `Beszéd / Cset` gombok alatt indul, hogy a beszélgetés feljebb kerüljön és több hely maradjon a buborékoknak.
...
A Felolvasó képernyő tetején ugyanaz a hírfolyamnézet-váltó jelenik meg, mint a Hírfolyamon: `Személyes / Időrend / Kedvelt`. A választás közös állapot, ezért a Hírfolyamon történt váltás a Felolvasóban is érvényes, és fordítva. A Felolvasó `Előző`, `Következő`, automatikus hírléptető és hangutasításos váltás műveletei mindig az aktuális hírfolyamnézet listáján belül dolgoznak. A nézetek hangutasítással is válthatók, például `Személyes`, `Időrend` vagy `Kedvelt hírek`.

A Felolvasó 30 elemű, hír-ID alapú lejátszási memóriát használ. Ez a ténylegesen lejátszott hírek sorrendjét őrzi, nem a feed aktuális időrendi indexét. Az `Előző` gomb vagy hangutasítás mindig az előzőleg lejátszott hírre lép. A `Következő` visszalépés után először a lejátszási memória bejárt útvonalán halad előre; ha a memória végén jár, akkor az aktuális feed következő még nem olvasott/felolvasott hírére ugrik. Frissítéskor az aktuális hír nem szakad meg, de a következő léptetés a feed tetején lévő legfrissebb, még nem felolvasott hírek felé folytatja.
...
Az Asszisztens Cset és promo cset nézete billentyűzet-overlay logikát használ. A virtuális billentyűzet megjelenése nem méretezheti át az egész appot, és nem tolhatja fel az alsó menüt a billentyűzet fölé. A prototípus a `visualViewport` és `VirtualKeyboard` jeléből számolja a billentyűzet alsó takarását; ilyenkor nem oldalsó menüs nézetben az alsó menü elrejtődik, az Asszisztens nézet a teljes alsó területig leenged, a cset és a teljes `.composer` beviteli box pedig a billentyűzet teteje fölé fut. A beviteli box felett és alatt alapnézetben 14 px természetes távolság marad. Mobil fekvő nézetben ez a kompakt szabály szerint 7 px. A végleges natív appban ugyanezt a platform billentyűzet-inset értéke alapján kell működtetni.

A Cset és promo cset buborékjai alulról építkeznek: ha kevés üzenet van, az új buborékok közvetlenül a beviteli mező fölött jelennek meg, nem a csetablak tetején. Hosszabb beszélgetésnél a régi üzenetek felfelé gördülnek ki, az új üzenet pedig mindig lent marad.

### Beállítások

A Beállítások főmenü első sora a `Csomagok és előfizetés`, mert ez a legfontosabb jogosultsági és csomagkezelési belépési pont. Ez alatt három logikai csoport jelenik meg:

- `Hírbeállítások`: RSS-források, Témák és érdeklődés, Helyi hírek, Értesítések.
- `Alkalmazás`: Fiók és biztonság, Megjelenés, Hang és felolvasó, Mobiladat és tárhely.
- `Prototípus`: Prototípus, CarPlay / Android Auto nézet.

A `CarPlay / Android Auto nézet` nem végfelhasználói főbeállítás, hanem prototípus-előnézet, ezért a Prototípus csoport alá tartozik, és az ideiglenes autós kijelző sheetet nyitja meg Android Auto és CarPlay fülekkel.

A főmenü sorai egységes, szemantikus inline SVG ikonokat használnak. Az ikonok vékony, lekerekített vonalvezetésűek, és finom coral akcentussal illeszkednek az értesítési menüikonok és a Hírbeszéd vizuális nyelvéhez.
...
- bekapcsolva: a hír végén hangutasítási ablakot nyit, de külön képernyőn látható parancslista nem jelenik meg;
...
A Felolvasó képernyőn nincs külön Hangutasítások kártya, mini súgó vagy látható parancslista. A parancsok az aktuális vezérlőgombok címeiből következnek, a Kedvelés paranccsal pedig az aktuális hír későbbre elmenthető.

A Felolvasó funkció indításakor rövid hangos útmutató hangzik el:

```text
Ha a mikrofon aktív, a Felolvasót hangutasításokkal is vezérelheted. Mondd ki annak a vezérlőgombnak a nevét, amit használni szeretnél, például szünet, következő hír vagy részletes hírek. A hírt a kedvelés paranccsal elmentheted magadnak későbbre.
```
...
Ha a mikrofon ki van kapcsolva, hangutasítás nem indul, de a képernyő nem mutat külön kikapcsolt parancslistát. Az alsó gombsor nem mozdulhat el attól, hogy a mikrofon be vagy ki van kapcsolva.
...
## 14. Helyi verziómentés

A fejlesztői környezet része a teljes helyi snapshot-rendszer. Célja, hogy egy elfogadott helyi prototípusállapot később külön mappából újra megnyitható, összehasonlítható és szükség esetén kontrolláltan visszaállítható legyen.
...
Alapértelmezett mentési gyökér:

```text
C:\Users\mulik\Documents\Fejlesztés\Hírbeszéd_verziok
```

A script teljes projektmásolatot készít, de kihagyja a fejlesztői és publikálási melléktermékeket: `.publish`, `.git`, `node_modules`, `.cache`, `_snapshot_meta`, `local-server.log`, `local-server.err.log`. A mentés saját `_snapshot_meta` mappát kap `SNAPSHOT_INFO.txt` és `SNAPSHOT_MANIFEST.tsv` fájllal.

Összehasonlítás a legutóbbi mentéssel:

```powershell
.\helyi_osszehasonlitas.cmd -Latest
```

Összehasonlítás konkrét mentéssel:

```powershell
.\helyi_osszehasonlitas.cmd -Snapshot "mentes-mappa-neve"
```

Régi állapot vizuális ellenőrzéséhez a mentett mappában külön statikus szerver indítható egy szabad porton, például `8010`-en. A visszaállítás nem automatikus parancs: először mindig új mentést kell készíteni az aktuális állapotról, és csak utána szabad fájlokat kontrolláltan visszahozni a kiválasztott snapshotból.

Ez helyi fejlesztői biztonsági rendszer, nem GitHub publikálás és nem nyilvános verziókiadás.

## 15. GitHub publikálás
...
## 16. Következő fejlesztési fókuszok
