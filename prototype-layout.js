(function(){
  ...
  function resetPhoneShellScroll(){
    var phone=document.querySelector('.phone-shell');
    if(!phone)return;
    if(phone.scrollTop)phone.scrollTop=0;
    if(phone.scrollLeft)phone.scrollLeft=0;
  }

  function schedulePhoneShellScrollReset(){
    resetPhoneShellScroll();
    (window.requestAnimationFrame||setTimeout)(resetPhoneShellScroll,0);
    setTimeout(resetPhoneShellScroll,60);
  }
  ...
  document.addEventListener('focusin',function(event){
    if(event.target&&event.target.closest&&event.target.closest('.bottom-nav'))schedulePhoneShellScrollReset();
  },true);
